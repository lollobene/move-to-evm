...
const make_test = function (move_contract, sol_contract) {
    return function () {
        it('CallMove', async function () {
            ...
            this.struct_abi_sol = await this.StructABI_Sol.deploy(
                this.abi_struct.address
            );
            const tx = this.struct_abi_sol.call_s();
            await expect(await tx)
                .to.emit(this.abi_struct, 'Event_u64')
                .withArgs(42);
        });
        ...
        it('CallSol', async function () {
            ...
            const tx = this.abi_struct.safe_transfer_form(
                this.struct_abi_sol.address
            );
            await expect(await tx)
                .to.emit(this.abi_struct, 'Event_u64')
                .withArgs(100);
        ...
        it('StructEvent', async function () {
            ...
            expectEvent(await this.abi_struct.do_transfer(), 'Event_S', [
                ['42', true, ['42']],
            ]);
        });
        ...
        it('StringEvent', async function () {
            ...
            this.abi_struct = await ABIStruct.new();
            expectEvent(
                await this.abi_struct.test_string([[97, 98, 99]]),
                'Event_String',
                [['0x616263']]
            );
        });
...
describe('Caller (the Move contract) NOT COMPLATIBLE ANYMORE, IT HAS BEEN REMOVED', () => {});
